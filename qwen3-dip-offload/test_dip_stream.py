"""
test_dip_stream.py — Selective-streaming DIP on offloaded 32B.
Usage:
    python test_dip_stream.py --ckpt checkpoints/qwen3-32b-int4 --gpu_mem 10 \
        --tokens 8 --keep 0.32
"""
import argparse, time
import torch
from qlinear import load_quantized_qwen3
from dip_stream import build_streaming_dip, _use_gather
...
def main():
    ...
    print("Building streaming DIP (loads up/down from checkpoint, pins on CPU) ...")
    mods = build_streaming_dip(model, args.ckpt, args.keep)
    print(f"Converted {len(mods)} MLPs to selective-streaming DIP")
    ...
    print(f"\nOutput:\n{txt}")
    print("\n" + "=" * 50)
    print(f"Streaming DIP: {ntok} tok in {dt:.2f}s -> {ntok/dt:.3f} tok/s  (baseline 0.088)")
    print("=" * 50)
