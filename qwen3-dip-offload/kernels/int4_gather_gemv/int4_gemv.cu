...
//
// Weight layout (must match quantize.py):
...
//
// Computes, per output row r:
//   out[r] = sum_c ( dequant(qweight[r,c]) * x[c] )
//   where dequant uses scales[r, c/128].
//
// One block per output row; block reduces over the input dimension.
// bf16 in/out. Built for sm_75 (Tesla T4).
...
__global__ void int4_dequant_gemv_kernel(
    ...
    // Each thread walks packed bytes; each byte covers 2 input columns.
