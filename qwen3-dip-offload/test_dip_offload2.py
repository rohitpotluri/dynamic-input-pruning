"""
test_dip_offload2.py — DIP via QuantLinear-level injection on offloaded 32B.
Keeps every module forward intact (accelerate-safe). Should produce coherent
output AND a speedup.

Usage:
    python test_dip_offload2.py --ckpt checkpoints/qwen3-32b-int4 --gpu_mem 10 \
        --tokens 8 --keep 0.32
"""
import argparse, time
import torch
from qlinear import load_quantized_qwen3
from dip_linear import convert_mlp_to_dip, _use_gather
...
def main():
    ...
    print("Converting MLPs to DIP (QuantLinear-level) ...")
    convert_mlp_to_dip(model, args.keep)
    ...
    print(f"\nOutput:\n{txt}")
    print("\n" + "=" * 50)
    print(f"DIP: {ntok} tok in {dt:.2f}s -> {ntok/dt:.3f} tok/s  (baseline 0.088)")
    print("=" * 50)
