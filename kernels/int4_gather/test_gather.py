"""
test_gather.py — Check the gather + int4-GEMV kernel.

Confirms that computing only the selected rows matches a full GEMV restricted
to those same rows. Run from kernels/int4_gather/ after building:
...
def main():
    ...
    # select a subset of rows at random (real DIP picks them by gate magnitude)
    k = int(out_f * 0.6)
    ...
    # reference: full dequant GEMV, then take the same rows
