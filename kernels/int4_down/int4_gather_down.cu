...
// down is stored transposed and int4-packed: down_t [inter, out], so channel c
// is a contiguous row down_t[c, :] of length `out`. DIP selects k channels, and
// only those rows are streamed into a contiguous buffer, so the row index is
// just 0..k-1:
//
//   y[o] = sum_{j=0..k-1} h_sel[j] * dequant(down_t_sel[j, o])
//
//   down_t_sel : int8 [k, out/2]     (k selected rows, two int4 per byte)
//   scales     : bf16 [k, out/128]
//   h_sel      : bf16 [k]            (gate * up for the selected channels)
//   y          : bf16 [out]
//
// One block per output element o; threads in the block split the k channels
// and reduce. Built for sm_75.
...
// One block per output element o: read down_t_sel[j, o] (int4) for each
// selected row j, dequantize, multiply by h_sel[j], accumulate.
__global__ void int4_gather_down_kernel(
    ...
    int o = blockIdx.x;                 // output element (0..out-1)
    int packed_col = o / 2;             // byte within the row
