"""
quantize.py — Quantize bf16 Qwen3-32B to symmetric int4, group-128, in the
layout the fused gather + dequant + GEMV kernels read directly.

  * Symmetric quant: value = round(w / scale), no zero-point.
    scale = max(|w_group|) / 7   (signed int4 range is [-8, 7]; scale by 7)
  ...
  * lm_head and embeddings stay in bf16 (accuracy-sensitive; lm_head is large
    but runs once per token at the end).

Usage:
    python scripts/quantize.py --model_dir checkpoints/Qwen/Qwen3-32B \
...
GROUP_SIZE = 128
QMAX = 7  # signed int4 range is [-8, 7]; scale by 7
...
def quantize_tensor_int4_groupwise(w: torch.Tensor):
    ...
    scales = (max_abs / QMAX).clamp(min=1e-8)              # avoid divide by zero

    # quantize to integers in [-8, 7]
...
def should_quantize(name: str) -> bool:
    """Quantize the attention and MLP projection weights; skip embeddings,
    norms, and lm_head."""
    ...
    if "lm_head" in lname:
        return False
    if "norm" in lname:          # RMSNorm weights (1D)
        return False
...
def main():
    ...
    # copy config + tokenizer so the checkpoint is self-contained
    ...
    # quant metadata for the loader to verify
    meta = {
